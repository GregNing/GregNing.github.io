...
categories: 
---
JWT是Json Web Token的縮寫，詳細規範在RFC7519中，目的是用來傳遞JSON物件並且透過雜湊簽章來確保資料沒有被變更過。因此我們可以把它當作驗證的token來用，也可以用來在兩個client之間傳遞資料。
...
Payload
Payload則用來存放用來要求資料的資訊(claims)，例如使用者名稱帳號等。claims有三種reserved claims，public claims及private claims
在這裡可以存放一些基本的驗證資訊，在RFC7519中也包含了基本內建的幾個資訊如iss(issuer)、sub(subject)等等(非必要)，當然我們也可以加入自己要的資訊，一個payload看起來會像這樣
...
Signature
Signature的是以編碼過的header，payload及密碼來產生，例如使用HMAC SHA256編碼，則簽名會以下面方式來建立。第一部分為header.payload的Base64Url編碼，第二部分為密碼（這邊的密碼為secret）。
...
在Startup.cs裡
	...
	public void Configuration(IAppBuilder app)
	{
		...
		{                
			...
			{
			new SymmetricKeyIssuerSecurityTokenProvider(issuer, Microsoft.Owin.Security.DataHandler.Encoder.TextEncodings.Base64Url.Decode(signkey))                        
			},
			...
			ValidIssuer = issuer,
			//認證SigningKey
			IssuerSigningKey = new InMemorySymmetricSecurityKey(Microsoft.Owin.Security.DataHandler.Encoder.TextEncodings.Base64Url.Decode(signkey)),
...
開始實作取得token
	...
	public static string setToken()
	{
		...
		InMemorySymmetricSecurityKey key = new InMemorySymmetricSecurityKey(Microsoft.Owin.Security.DataHandler.Encoder.TextEncodings.Base64Url.Decode(signKey));                
...
驗證Token資訊
	...
	private bool VerifyToken(string token)
	{
		...
		{
			IssuerSigningToken = new System.ServiceModel.Security.Tokens.BinarySecretSecurityToken(Microsoft.Owin.Security.DataHandler.Encoder.TextEncodings.Base64Url.Decode(signkey)),
			ValidAudience = audience,
			ValidIssuer = issuer,
...
發生IDX10603: The 'System.IdentityModel.Tokens.InMemorySymmetricSecurityKey' cannot have less than: '128' bits. Parameternavn: key.KeySize The actual size was 48錯誤
